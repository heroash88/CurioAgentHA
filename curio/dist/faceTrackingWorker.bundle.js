...
(() => {
  ...
  var extractNormalizedPoint = (candidate, frameWidth, frameHeight) => {
  ...
  var extractNormalizedCenter = (box, frameWidth, frameHeight) => {
    if (!box) return null;
    if (typeof box.xCenter === "number" && typeof box.yCenter === "number") {
      const x2 = normalizeCoordinate(box.xCenter, frameWidth);
      const y2 = normalizeCoordinate(box.yCenter, frameHeight);
